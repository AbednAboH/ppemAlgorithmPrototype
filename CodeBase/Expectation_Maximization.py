...
class algortithem:
    ...
    def __init__(self, n: int, inputDimentions: int = 2, max_iter: int = 100, number_of_clustures: int = 2,
                 ...
                 plottingTools: bool = False, plot_name="", show_time=True, id=0, plot_last_iter=True,name="Expectation Maximization"):
    ...
    def handle_prints_time(self):
        ...
        print_time((self.iteration,self.log_likelihoods[-1],runtime, clockticks))
    ...
    def solve(self):
        """
        Skeleton for all algorithms derived from this class, it includes\
        ...
        for i in range(self.max_iter):
            ...
            self.iter.append(i)
            if self.show_time: self.handle_prints_time()
    ...
    def usePlotingTools(self, iteration, bool):
        ...
        try:
            ...
            twoDimentionalGifCreator(self.n_inputs, self._means, self._covariances, self.k, iteration, self.plots,
                                         self._pi, self.plot_name,gif=self.plottingEnabled)
        except np.linalg.LinAlgError:
            print(f"ERROR !{self.plot_name}  Singular matrix cannot be inversed! ")
        finally:
            pass
...
# prints time
print_time = lambda x: print(f"Iteration {x[0]} |  Log Likelyhood : {x[1]}  | Time :  {x[2]}  |  ticks: {x[3]}")
# calculates variance
