...
class PPEM(algortithem):
    ...
    def eStep(self,X, means, covariances, weights, public_key):
        ...
        for i in range(n_components):
            ...
            for j in range(n_samples):
                x = X[j]
                x_encrypted = encrypt_data(public_key, x)
                mean_encrypted = encrypt_data(public_key, mean)
                covariance_inv_encrypted = encrypt_data(public_key, covariance_inv)
                quadratic_form = (x_encrypted - mean_encrypted).dot(covariance_inv_encrypted).dot(
                    x_encrypted - mean_encrypted)
                log_probabilities[j, i] = np.log(weight) - 0.5 * np.log(covariance_det) - 0.5 * quadratic_form
    ...
    def mstep(self,X, gamma, k, n, m, public_key, secret_key):
        ...
        for i in range(k):
            ...
            mu_i = np.sum(gamma[:, i].reshape(-1, 1) * X, axis=0) / (np.sum(gamma[:, i]) + epsilon)
            mu.append(mu_i)
            ...
            mu_i_encrypted = bfv_encrypt(mu_i, public_key)
            cov_i_encrypted = bfv_encrypt(cov_i, public_key)
            pi_i_encrypted = bfv_encrypt(pi_i, public_key)
